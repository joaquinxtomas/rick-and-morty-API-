...
const getList = async (url) => {
    try {
        ...
        btnNext = results.info.next ? `<a class="btn" data-url=${results.info.next}>Siguiente</a>` : ''
        btnPrevious = results.info.prev ? `<a class="btn" data-url=${results.info.prev}>Anterior</a>` : ''
    ...
    try {
        for (const info of data) {
            ...
            <div class="desplegable">
                <p>Género: ${resul.gender}</p>
                <p>Origen: ${resul.origin.name}</p>
                <p>Primera aparición:<br><a href="#">${resul.episode[0]}</a></p>
                </div>
            </div>
            </div>`
...
let formSearch = form.addEventListener('submit',(e)=>{
    ...
    const character = async (character) => {
        ...
        try {
                ...
                <p>Primera aparición:<br><a href="#">${resultadoPersonaje.results[index].episode[0]}</a></p>
                </div>
            </div>
                </div>`
                ...
                let boton = document.querySelector('.boton-buscar')
